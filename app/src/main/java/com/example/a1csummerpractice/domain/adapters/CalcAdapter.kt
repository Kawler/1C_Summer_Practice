...
import com.example.a1csummerpractice.databinding.ItemChartRbBinding
import com.example.a1csummerpractice.domain.dtypes.Item
...
class CalcAdapter : RecyclerView.Adapter<CalcAdapter.CalcViewHolder>() {
    ...
    class CalcViewHolder(val binding: ItemChartRbBinding) :
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): CalcViewHolder {
        ...
        val binding = ItemChartRbBinding.inflate(inflater, parent, false)
    ...
    override fun onBindViewHolder(holder: CalcViewHolder, position: Int) {
        ...
        with(holder.binding) {
            rvItemDate.text = "Дата: " + SimpleDateFormat("dd/MM/yyyy").format(Date(item.date*1000))
            ...
            rvItemCurrentDate.text = "Текущая дата: " + SimpleDateFormat("dd/MM/yyyy").format(Date(item.currentDate*1000))
